...
class UpdateHelper():
    ...
    async def get_player_data(self):
        print('[{0} UpdateHelper] Running player data update, either this is the first update or last update was over 7 days ago (lu: {1})'\
                        .format(self.queried_player.name, self.queried_player.last_data_update))
    ...
    async def get_player_ranked_data(self):
        ...
        print('[{0} UpdateHelper] Running SoloQ data update.'\
    ...
    async def get_streak_data(self):
        ...
        print('[{0} UpdateHelper] Running streak data update'\
                        .format(self.queried_player.name))
        tasks = []
        matches = []
        result = last_result = None
        streak = 0
        history = await lol.MatchHistory(self.queried_player.puuid, region=self.queried_player.region).query(count=10, queue=420).get()
